...
INSERT into PATIENT (NAME, VORNAME, KRANKENKASSENKUERZEL, VERSICHERTENNR, ANSCHRIFT, GEBURTSDATUM)
    values ('Friedrich', 'Bob', 'AOK', 1, 'Bobstraße 3, Hamburg', TO_DATE('2003/07/09', 'yyyy/mm/dd'));

INSERT into PATIENT (NAME, VORNAME, KRANKENKASSENKUERZEL, VERSICHERTENNR, ANSCHRIFT, GEBURTSDATUM)
    values ('Gunthilde', 'Edeltraudt', 'TK', 5656, 'Edelstraße 6', TO_DATE('456/07/09', 'yyyy/mm/dd'));

INSERT into PATIENT (NAME, VORNAME, KRANKENKASSENKUERZEL, VERSICHERTENNR, ANSCHRIFT, GEBURTSDATUM)
    values ('Friedrich', 'Bobba', 'AOK', 2, 'Bobstraße 3, Hamburg', TO_DATE('2003/07/29', 'yyyy/mm/dd'));
...
SELECT Vorname, Name, Anschrift
FROM PATIENT 
WHERE ANSCHRIFT 
LIKE '%Hamburg';

SELECT FPK.BEZEICHNUNG, FPK.DRG, BH.STARTDATUM, BH.ABGERECHNETAM 
FROM FALLPAUSCHALENK FPK, AUFENTHALT BH
WHERE BH.DRG = FPK.DRG
ORDER BY FPK.DRG;

SELECT Bezeichnung AS Station, Vorname, Name, PersonalNr, Geburtsdatum, (SELECT Name FROM Personal WHERE LeitetStationsNr = Station.StationsNr) AS LeitungsName FROM Personal, Station
WHERE Station.StationsNr = Personal.ArbeitetInStationsNr;

SELECT Zimmerbelegung.ZimmerNr, Patient.Name, Patient.Vorname, Patient.Geburtsdatum FROM Zimmerbelegung, Patient, Zimmer
    WHERE Zimmerbelegung.PatientenNr = Patient.PatientenNr AND Zimmerbelegung.ZimmerNr = Zimmer.ZimmerNr AND
        (Zimmerbelegung.Von <= TO_DATE('20220401', 'yyyymmdd') AND Zimmerbelegung.Bis IS NULL OR Zimmerbelegung.Von <= TO_DATE('20220401', 'yyyymmdd') AND Zimmerbelegung.Bis >= TO_DATE('20220401', 'yyyymmdd'));
